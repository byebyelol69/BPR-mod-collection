...
def unpack_multiple_bundles(bundle_dir, main_save_directory, unpack_to_same_folder=False):
	#wrong_ones = 0
	#right_ones = 0
...
def manual_command_handler(command):
	if command == "-h" or command == "--help":
		...
		print("by DGIorio")
		print()
		print("Usage: python %s [option] <input_file> <output_dir> <output_name>" % (sys.argv[0]))
		print()
		print("Options:")
		...
		if os.path.isfile(input_arg):
			...
			print("")
			status = unpack_multiple_bundles(input_arg, output_dir, unpack_to_same_folder)
...
if __name__ == "__main__":
	...
	if len(sys.argv) == 1:
		...
		print("by DGIorio")
		print()
		print("Command-line usage: python %s [option] <input_file> <output_dir> <output_name>" % (sys.argv[0]))
		print()
		print("Options:")
		print("  -h, --help      Show this help message")
		print("  -v, --version   Show the tool version")
		...
		if len(sys.argv)-1 != 3:
			print("Error: insuficient arguments. You must specify an input file and an output directory")
		elif os.path.isfile(sys.argv[3]):
			print("Error: invalid argument. <output_dir> must be an directory")
		else:
			if os.path.isfile(sys.argv[2]):
				if game.lower() == "bp":
					status = unpack_bundle(sys.argv[2], sys.argv[3], "IDs_" + os.path.basename(sys.argv[2]))
				elif game.lower() == "mw":
					status = unpack_bundle_mw(sys.argv[2], sys.argv[3], "IDs_" + os.path.basename(sys.argv[2]))
				print("Info: finished unpacking file")
			elif os.path.isdir(sys.argv[2]):
				status = unpack_multiple_bundles(sys.argv[2], sys.argv[3])
				print("Info: finished unpacking files")
	
	elif sys.argv[1] == "-p" or sys.argv[1] == "--pack":
		if len(sys.argv)-1 != 4:
			print("Error: insuficient arguments. You must specify an input file, an output directory and an output file name")
		elif os.path.isfile(sys.argv[3]):
			print("Error: invalid argument. <output_dir> must be an directory")
		else:
			if os.path.isfile(sys.argv[2]):
				if game.lower() == "bp":
					status = pack_bundle(sys.argv[2], sys.argv[3], sys.argv[4])
				elif game.lower() == "mw":
					status = pack_bundle_mw(sys.argv[2], sys.argv[3], sys.argv[4])
				print("Info: finished packing file")
			elif os.path.isdir(sys.argv[2]):
				#status = pack_multiple_bundles(sys.argv[2], sys.argv[3], sys.argv[4])
				print("Error: packing multiple bundles is not supporter yet")
	else:
		print("Error: invalid argument. Please use the argument -h for help")
